def content(makeFns):
	...
	w("<pre>\n")
	w("<code><em>filename</em> ::= <em>year</em> \".\" <em>documentNumber</em> \".\" <em>paragraphNumber</em> \".\" <em>tableType</em> \".\" <em>operationType</em> [ \"(\" <em>operationArguments</em> \")\" ] \".csv\"</code>\n")
	w("<code><em>year</em> ::= <em>число</em></code>\n")
	...
	w("<code><em>operationType</em> ::= <em>строка</em></code>\n")
	w("<code><em>operationArguments</em> ::= <em>число</em> { \",\" <em>число</em> }</code>\n")
	w("</pre>\n")
	...
	wDefCodeList((
		("<em>year</em>","основной год, для которого рассматривается бюджет (пока всегда 2014); не является годом планового периода"),
